...
- The DLAO Overview and New/Incoming worklists run every received application through `classifyIncomingComplexity()` (`SIMULATED_COMPLEXITY_V1`) and render three subsections in priority order: Complex, Intermediate, Petty/easy. The deterministic simulation scores existing structured facts only: red/urgent/high priority, sensitive or land matter, child involvement, multiple urgency/incident signals, local identity verification, representative filing, pending documents and a long narrative. Each row displays concise matched reasons. The result is `simulated: true`, `advisoryOnly: true`, and changes only queue presentation; it cannot decide eligibility, urgency, pathway or outcome.
