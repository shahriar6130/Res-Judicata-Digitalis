...
function CitizenDashboard({ lang }: { lang: Lang }) {
  ...
  return (
    <div className={styles.page}>
      ...
      />
      <section id="cases" className={styles.heroStatus}>
        <div>
          ...
          <p>{lang === "bn" ? "তারিখটি আদালতের নথি দেখে যাচাই করা হয়েছে। সকাল ১০টার মধ্যে জেলা আদালতে উপস্থিত থাকুন।" : "The date was verified against a court record. Arrive at the District Court by 10:00 AM."}</p>
        </div>
      ...
      <div className={styles.twoColumn}>
        <section id="messages" className={styles.section}>
          <div className={styles.sectionHeading}>
            <div><p className={styles.sectionLabel}>{lang === "bn" ? "সর্বশেষ বার্তা" : "Latest message"}</p><h2>{lang === "bn" ? "পরবর্তী পদক্ষেপ নিশ্চিত করুন" : "Confirm your next step"}</h2></div>
            <span className={styles.simTag}>{copy(labels.simulated, lang)}</span>
          </div>
          <p className={styles.message}>{lang === "bn" ? "আপনার শুনানির তারিখ ৩০ সেপ্টেম্বর। এই তথ্যটি কি আপনার জানা তথ্যের সঙ্গে মেলে?" : "Your hearing date is 30 September. Does this match the information you have?"}</p>
          {reply ? <p role="status" className={styles.success}>{reply === "confirm" ? (lang === "bn" ? "আপনার নিশ্চিতকরণ নথিভুক্ত হয়েছে।" : "Your confirmation was recorded.") : reply === "dispute" ? (lang === "bn" ? "বিষয়টি কর্মকর্তার পর্যালোচনার জন্য পাঠানো হয়েছে।" : "This was sent to an officer for review.") : (lang === "bn" ? "ফোন করার অনুরোধ নথিভুক্ত হয়েছে।" : "Your callback request was recorded.")}</p> : null}
          <div className={styles.actions}>
            <Button onClick={() => setReply("confirm")}>{lang === "bn" ? "তথ্য ঠিক আছে" : "Confirm"}</Button>
            <Button variant="secondary" onClick={() => setReply("dispute")}>{lang === "bn" ? "তথ্যটি ভুল" : "Dispute"}</Button>
            <button className={styles.textButton} onClick={() => setReply("callback")}>{lang === "bn" ? "আমাকে ফোন করুন" : "Request a call"}</button>
          </div>
        </section>

        <aside id="profile" className={styles.facts}>
          <h2>{lang === "bn" ? "মামলার তথ্য" : "Case facts"}</h2>
          <dl><div><dt>{lang === "bn" ? "অবস্থা" : "Status"}</dt><dd>{lang === "bn" ? "আইনি সহায়তা চলমান" : "Legal aid active"}</dd></div><div><dt>{lang === "bn" ? "আইনজীবী" : "Lawyer"}</dt><dd>{lang === "bn" ? "ফারহানা রহমান" : "Farhana Rahman"}</dd></div><div><dt>{lang === "bn" ? "পরবর্তী কাজ" : "Next action"}</dt><dd>{lang === "bn" ? "শুনানিতে উপস্থিতি" : "Attend hearing"}</dd></div></dl>
          <button className={styles.textButton} onClick={() => setDetails(!details)}>{copy(labels.view, lang)}</button>
          {details ? <p className={styles.evidence}>{lang === "bn" ? "উৎস: সিমুলেটেড আদালত নথি · ধারণ: ২০ সেপ্টেম্বর ২০২৬ · নীতি: hearing-date/v1" : "Source: simulated court record · captured 20 Sep 2026 · policy: hearing-date/v1"}</p> : null}
        </aside>
      </div>
    </div>
  );
