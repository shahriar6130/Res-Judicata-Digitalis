...
exports.write = write;
exports.clear = clear;
exports.useHelplineStore = useHelplineStore;
const event_bus_1 = require("./event-bus");
exports.STORE_KEY = "shakkho.helpline.v1";
...
function notDemo(list) {
    if (!Array.isArray(list))
        return list;
    return list.filter((x) => {
        const r = x;
        return !((r.id && LEGACY_UDC_DEMO_IDS.has(r.id)) ||
...
function read() {
    ...
    try {
        const raw = window.localStorage.getItem(exports.STORE_KEY);
        if (!raw)
            return emptyEnvelope();
...
function shallowEqual(a, b) {
    if (a === b)
        return true;
    if (a.seededAt !== b.seededAt)
        return false;
    if (a.records.length !== b.records.length)
        return false;
    if (a.sessions.length !== b.sessions.length)
        return false;
    if (a.representations.length !== b.representations.length)
        return false;
    if (a.verifications.length !== b.verifications.length)
        return false;
    if (a.handoffs.length !== b.handoffs.length)
        return false;
    if (a.communications.length !== b.communications.length)
        return false;
    if (a.audit.length !== b.audit.length)
        return false;
    if (a.dlaoTasks.length !== b.dlaoTasks.length)
        return false;
    if (a.activeCallBar !== b.activeCallBar)
        return false;
    if (a.humanIntakes.length !== b.humanIntakes.length)
        return false;
    if (a.dlaoVerifications.length !== b.dlaoVerifications.length)
        return false;
    if (a.safeContactPlans.length !== b.safeContactPlans.length)
        return false;
    if (a.referrals.length !== b.referrals.length)
        return false;
    if (a.escalationTasks.length !== b.escalationTasks.length)
        return false;
    if (a.deliveryOperations.length !== b.deliveryOperations.length)
        return false;
    if (a.lawyerAssignments.length !== b.lawyerAssignments.length)
        return false;
    if (a.caseHearings.length !== b.caseHearings.length)
        return false;
    if (a.caseProgressUpdates.length !== b.caseProgressUpdates.length)
        return false;
    if (a.requiredUpdates.length !== b.requiredUpdates.length)
        return false;
    if (a.contactAttempts.length !== b.contactAttempts.length)
        return false;
    if (a.lawyerChangeRequests.length !== b.lawyerChangeRequests.length)
        return false;
    if (a.inactivityPatterns.length !== b.inactivityPatterns.length)
        return false;
    if (a.paymentReconciliations.length !== b.paymentReconciliations.length)
        return false;
    if (a.voiceStatusSessions.length !== b.voiceStatusSessions.length)
        return false;
    if (a.dlaoTaskItems.length !== b.dlaoTaskItems.length)
        return false;
    if (a.mediationMatters.length !== b.mediationMatters.length)
        return false;
    if (a.mediationSessions.length !== b.mediationSessions.length)
        return false;
    if (a.settlementDrafts.length !== b.settlementDrafts.length)
        return false;
    if (a.signingWorkflows.length !== b.signingWorkflows.length)
        return false;
    if (a.demoTimeOffsetMs !== b.demoTimeOffsetMs)
        return false;
    // In-place field changes in the mediation arrays (no length change).
    if (mediationSignature(a) !== mediationSignature(b))
        return false;
    return true;
}
function write(next) {
    ...
    if (shallowEqual(next, currentSnapshot))
        return;
    ...
    writeTimer = setTimeout(() => {
        try {
            window.localStorage.setItem(exports.STORE_KEY, JSON.stringify(next));
            (0, event_bus_1.publish)(next);
        }
        catch {
...
function clear() {
    if (!isBrowser())
        return;
    try {
        window.localStorage.removeItem(exports.STORE_KEY);
    }
    catch {
        // ignore
    }
...
function useHelplineStore() {
    return (0, react_1.useSyncExternalStore)((onChange) => {
        function onStorage(event) {
            if (event.key === exports.STORE_KEY) {
        ...
        window.addEventListener(event_bus_1.STORE_EVENT, onStoreEvent);
        ...
        return () => {
            ...
            window.removeEventListener(event_bus_1.STORE_EVENT, onStoreEvent);
        };
