...
const FORMALITY_WARNING: Record<SettlementDraftCategory, { bn: string; en: string }> = {
  maintenance: {
...
const TEMPLATES: Record<SettlementDraftCategory, { templateId: string; clauses: Omit<SettlementClause, "clauseId" | "origin" | "sourceNote" | "required" | "disposition">[] }> = {
  maintenance: {
...
function fillTemplate(body: string, fields: Record<string, string | undefined>): { text: string; origin: SettlementClause["origin"]; sourceNote?: string } {
  ...
  if (!hadPlaceholder) return { text, origin: "template" };
  if (allFilled) return { text, origin: "ai_inferred" };
  if (anyFilled) return { text, origin: "ai_inferred", sourceNote: "Some fields could not be confidently extracted from the mediator's notes and are flagged inline rather than guessed." };
...
function extractFields(notes: string): Record<string, string> {
...
export const SettlementDraftingService = {
  list(envelope: StoreEnvelope): SettlementDraft[] {
    return envelope.settlementDrafts ?? [];
  },

  find(envelope: StoreEnvelope, draftId: string): SettlementDraft | undefined {
    return (envelope.settlementDrafts ?? []).find((d) => d.draftId === draftId);
  },

  forMatter(envelope: StoreEnvelope, matterId: string): SettlementDraft[] {
  ...
  generateDraft(input: {
    ...
    const clauses: SettlementClause[] = template.clauses.map((c, idx) => {
      const bn = fillTemplate(c.bodyBn, fields);
      const en = fillTemplate(c.bodyEn, fields);
      const origin: SettlementClause["origin"] = bn.origin === "ai_inferred" || en.origin === "ai_inferred" ? "ai_inferred" : "template";
      return {
        ...
        required: true,
        disposition: "undisposed" as ClauseDisposition,
      };
    });
    ...
    const audit = AuditTrailService.log(
      { subject: draftId, subjectKind: "settlement_draft", action: "settlement_draft.generated", actor: input.actor, payload: { matterId: input.matterId, category: input.category, inconsistencyCount: draft.inconsistencies.length } },
      { kind: "system", simulatedAt: now, note: { bn: "খসড়াটি অনুমোদিত টেমপ্লেট থেকে তৈরি; মধ্যস্থতাকারীর নোট থেকে ভরা তথ্য দৃশ্যমানভাবে চিহ্নিত।", en: "Draft generated from an approved template; fields filled from mediator notes are visibly flagged." } },
    );
    draft.auditEventIds = [audit.id];

    const fresh = read();
    write({ ...fresh, settlementDrafts: [...(fresh.settlementDrafts ?? []), draft] });
    return draft;
  },
   ...
   */
  runInconsistencyCheck(draft: SettlementDraft): SettlementInconsistency[] {
    const findings: SettlementInconsistency[] = [];

    const amounts: { clauseId: string; value: number }[] = [];
    for (const clause of draft.clauses) {
      const matches = clause.bodyEn.match(/\b(\d[\d,]{2,})\b/g) ?? [];
      for (const m of matches) {
        const value = Number(m.replace(/,/g, ""));
        if (!Number.isNaN(value)) amounts.push({ clauseId: clause.clauseId, value });
      }
    }
    if (amounts.length >= 2) {
  ...
  dispositionClause(input: {
    ...
    editedBodyEn?: string;
    actor: string;
  }): ServiceResult<SettlementDraft> {
    const envelope = read();
    const draft = (envelope.settlementDrafts ?? []).find((d) => d.draftId === input.draftId);
    if (!draft) return { ok: false, error: `No settlement draft ${input.draftId}.` };
    const now = DemoTimeService.iso();

    const clauses = draft.clauses.map((c) =>
      c.clauseId === input.clauseId
        ? {
            ...c,
            ...
            origin: input.disposition === "edit" ? ("human_edited" as const) : c.origin,
          }
        : c,
    );
    ...
    const audit = AuditTrailService.log(
      { subject: input.draftId, subjectKind: "settlement_draft", action: "settlement_draft.clause_dispositioned", actor: input.actor, payload: { clauseId: input.clauseId, disposition: input.disposition } },
      { kind: "officer_lookup", simulatedAt: now },
    );

    const updated: SettlementDraft = { ...draft, clauses, inconsistencies, version: draft.version + 1, auditEventIds: [...draft.auditEventIds, audit.id], updatedAt: now };
    const fresh = read();
    write({ ...fresh, settlementDrafts: (fresh.settlementDrafts ?? []).map((d) => (d.draftId === input.draftId ? updated : d)) });
    return { ok: true, value: updated };
  },

  allRequiredClausesDisposed(draft: SettlementDraft): boolean {
    return draft.clauses.filter((c) => c.required).every((c) => c.disposition !== "undisposed");
  },

  hasBlockingInconsistency(draft: SettlementDraft): boolean {
  ...
  updateConsentChecklist(input: { draftId: string; checklist: Partial<SettlementConsentChecklist>; actor: string }): ServiceResult<SettlementDraft> {
    const envelope = read();
    const draft = (envelope.settlementDrafts ?? []).find((d) => d.draftId === input.draftId);
    if (!draft) return { ok: false, error: `No settlement draft ${input.draftId}.` };
    const now = DemoTimeService.iso();
    ...
    const audit = AuditTrailService.log(
      { subject: input.draftId, subjectKind: "settlement_draft", action: "settlement_draft.checklist_updated", actor: input.actor, payload: input.checklist },
      { kind: "officer_lookup", simulatedAt: now },
    );

    const updated: SettlementDraft = { ...draft, consentChecklist, auditEventIds: [...draft.auditEventIds, audit.id], updatedAt: now };
    const fresh = read();
    write({ ...fresh, settlementDrafts: (fresh.settlementDrafts ?? []).map((d) => (d.draftId === input.draftId ? updated : d)) });
    return { ok: true, value: updated };
  },

  checklistComplete(checklist: SettlementConsentChecklist): boolean {
    return Object.values(checklist).every(Boolean);
  },

  submitForReview(input: { draftId: string; reviewer: string; notes?: string }): SettlementDraft | undefined {
    const envelope = read();
    const draft = (envelope.settlementDrafts ?? []).find((d) => d.draftId === input.draftId);
    ...
    const audit = AuditTrailService.log(
      { subject: input.draftId, subjectKind: "settlement_draft", action: "settlement_draft.human_reviewed", actor: input.reviewer, payload: { notes: input.notes } },
      { kind: "officer_lookup", simulatedAt: now },
    );

    const updated: SettlementDraft = {
      ...draft,
  ...
  requestPartyConsent(input: { draftId: string; actor: string }): ServiceResult<SettlementDraft> {
    const envelope = read();
    const draft = (envelope.settlementDrafts ?? []).find((d) => d.draftId === input.draftId);
    if (!draft || draft.status !== "human_reviewed") return { ok: false, error: "Draft must be human-reviewed before party review can open." };
    ...
    const audit = AuditTrailService.log(
      { subject: input.draftId, subjectKind: "settlement_draft", action: "settlement_draft.party_consent_pending", actor: input.actor },
      { kind: "officer_lookup", simulatedAt: now },
    );

    const updated: SettlementDraft = { ...draft, status: "party_consent_pending", auditEventIds: [...draft.auditEventIds, audit.id], updatedAt: now };
    const fresh = read();
    write({ ...fresh, settlementDrafts: (fresh.settlementDrafts ?? []).map((d) => (d.draftId === input.draftId ? updated : d)) });
    return { ok: true, value: updated };
  },
  ...
  recordPartyConsent(input: {
    ...
    party: string;
    consented: boolean;
    method: SettlementConsent["method"];
    actor: string;
  }): ServiceResult<SettlementDraft> {
    const envelope = read();
    const draft = (envelope.settlementDrafts ?? []).find((d) => d.draftId === input.draftId);
    ...
    const audit = AuditTrailService.log(
      { subject: input.draftId, subjectKind: "settlement_draft", action: "settlement_draft.party_consent_recorded", actor: input.actor, payload: { party: input.party, consented: input.consented, method: input.method } },
      { kind: "officer_lookup", simulatedAt: now },
    );
  ...
  renderDraftText(draft: SettlementDraft, lang: "bn" | "en" = "en"): string {
   ...
   */
  async finalize(input: { draftId: string; actor: string }): Promise<ServiceResult<SettlementDraft>> {
    const envelope = read();
    const draft = (envelope.settlementDrafts ?? []).find((d) => d.draftId === input.draftId);
    if (!draft) return { ok: false, error: `No settlement draft ${input.draftId}.` };
    ...
    const audit = AuditTrailService.log(
      { subject: input.draftId, subjectKind: "settlement_draft", action: "settlement_draft.finalized", actor: input.actor, payload: { frozenTextHash } },
      { kind: "officer_lookup", simulatedAt: now },
    );
   ...
   */
  invalidateAfterChange(input: { draftId: string; actor: string; reason: string }): ServiceResult<SettlementDraft> {
    const envelope = read();
    const draft = (envelope.settlementDrafts ?? []).find((d) => d.draftId === input.draftId);
    if (!draft) return { ok: false, error: `No settlement draft ${input.draftId}.` };
    const now = DemoTimeService.iso();
    ...
    const audit = AuditTrailService.log(
      { subject: input.draftId, subjectKind: "settlement_draft", action: "settlement_draft.invalidated_content_changed", actor: input.actor, payload: { reason: input.reason, previousHash: draft.frozenTextHash } },
      { kind: "officer_lookup", simulatedAt: now },
    );
